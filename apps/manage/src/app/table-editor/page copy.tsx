...
// 引入 Aitable 组件库样式，确保按钮/下拉等样式正确显示
...
export default function TableEditor() {
  ...
  // 恢复认证状态
  const { accessToken, refreshToken, isAuthenticated, user } = useAuthStore();
  ...
  // 准备视图数据
  const viewsData = useMemo(() => views.map(v => ({
    id: v.id,
    name: v.name,
    type: v.type,
  })), [views]);

  // 构建 Grid Props - 移动到所有函数定义之后
  const gridProps: IGridProps & { onDataRefresh?: () => void } = useMemo(() => ({
    columns: gridColumns,
    ...
    // 数据刷新回调（用于添加记录后刷新）
    onDataRefresh: () => {
      console.log('🔄 Refreshing table data after record creation...');
      if (tableId) {
        loadTableData(tableId, viewId);
      }
    },
  ...
  }), [gridColumns, records.length, getCellContent, handleCellEdited, handleAddColumn, handleEditColumn, handleDuplicateColumn, handleDeleteColumn, handleStartEditColumn]);
  ...
  const handleCreateView = useCallback(async (type: 'grid' | 'kanban') => {
    ...
    try {
      ...
      console.log('🔍 正在创建视图:', { tableId, name, type });
  ...
  return (
    <TableEditorLayout>
        ...
        <div className="flex-1 flex flex-col min-h-0">
          {baseId && tableId && viewId ? (
            ...
            >
              <StandardDataView
                gridProps={gridProps}
                showHeader={false} // 我们已经有了自定义的头部
                showToolbar={showToolbar}
                showStatus={true}
                // 传入字段以启用"配置字段"入口和内置"添加记录"弹窗
                fields={fields.map(f => ({
                  id: f.id,
                  name: f.name,
                  type: f.type as any,
                  visible: true,
                  required: false, // TODO: 从字段定义中获取
                  isPrimary: f.primary || false,
                  description: f.description || '',
                  options: f.options || {},
                }))}
                // 新增：传入 tableId 以启用内置"添加记录"弹窗
                // @ts-ignore - 临时忽略类型错误，等待组件更新
                tableId={tableId}
                sdk={luckdb}
                views={viewsData}
                activeViewId={view?.id}
                ...
                onAdd={() => {
                  console.log('[Record] AddNew clicked - open AddRecordDialog');
                  setAddRecordDialogOpen(true);
                }}
                // 最简配置：不传任何工具栏回调，让组件自动处理所有功能
                // 组件会自动显示"添加字段"按钮（因为传入了 sdk + tableId）
                // 组件会自动显示"添加记录"按钮（因为传入了 sdk + tableId）
                // onToolbar 回调全部移除，使用组件内置功能
                statusContent={
      ...
      {/* 记录管理弹窗：已由 @luckdb/aitable 内置，无需外部弹窗 */}
    </TableEditorLayout>
  );
}
