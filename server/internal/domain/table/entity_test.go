...
func TestField_CanChangeTypeTo(t *testing.T) {
	...
	canChange, err := fields.CanChangeTypeTo(fields.FieldTypeEmail)
	if err != nil {
		t.Errorf("Expected no error, got %v", err)
	}
	if !canChange {
	...
	canChange, err = primaryField.CanChangeTypeTo(fields.FieldTypeText)
	if err == nil {
...
func TestFieldType_IsCompatibleWith(t *testing.T) {
	// 测试兼容的类型转换
	if !FieldTypeText.IsCompatibleWith(fields.FieldTypeEmail) {
		t.Error("Expected text to email to be compatible")
	}

	if !FieldTypeNumber.IsCompatibleWith(fields.FieldTypeCurrency) {
		t.Error("Expected number to currency to be compatible")
	}

	// 测试不兼容的类型转换
	if FieldTypeText.IsCompatibleWith(fields.FieldTypeNumber) {
		t.Error("Expected text to number to be incompatible")
	}

	// 测试相同类型
	if !FieldTypeText.IsCompatibleWith(fields.FieldTypeText) {
...
func TestField_incrementVersion(t *testing.T) {
	...
	initialVersion := fields.Version
	field.incrementVersion()

	if fields.Version != initialVersion+1 {
		t.Errorf("Expected version to increment from %d to %d, got %d", initialVersion, initialVersion+1, fields.Version)
	}

	if fields.LastModifiedTime == nil {
		t.Error("Expected LastModifiedTime to be set")
	}

	// 验证时间是最近的
	timeDiff := time.Since(*fields.LastModifiedTime)
