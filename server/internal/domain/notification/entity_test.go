...
func TestNotificationTemplate_SetDefaultData(t *testing.T) {
	...
	// 设置默认数据
	template.SetDefaultData("platform_name", "LuckDB")
	template.SetDefaultData("version", "1.0.0")

	if template.DefaultData["platform_name"] != "LuckDB" {
		t.Errorf("SetDefaultData() platform_name = %v, want %v", template.DefaultData["platform_name"], "LuckDB")
	}
